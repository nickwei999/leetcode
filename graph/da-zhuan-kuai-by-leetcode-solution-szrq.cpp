...
/* 并查集逆向思维：
1、拷贝原砖块状态，将要击碎的砖块位置置空；
2、建立并查集；其中第一排关联到特殊的根节点： h*w;并记录特殊节点下的砖块个数；
3、逆序将击碎的位置补全，并遍历周边节点更新并查集；
新增的特殊节点下的砖块个数即因为敲碎掉落下来的砖块个数；

时间复杂度：O(n⋅hw⋅log(hw))，其中 n 为消除砖块的次数（即数组 hits 的长度），
h 和 w 分别为矩阵的行数和列数。图中共有 hw+1
个节点，故并查集的单次操作的时间复杂度为O(hw⋅log(hw))。 */
