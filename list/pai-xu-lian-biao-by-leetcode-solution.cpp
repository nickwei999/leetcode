   ...
   public:
    ListNode* sortList(ListNode* head) {
        ...
        while (lastSorted && lastSorted->next) {
            ...
            auto resortNode = lastSorted->next;
            lastSorted->next = resortNode->next;
            auto node = dummyHead;
            while (node->next->val < resortNode->val) {
                node = node->next;
            }
            resortNode->next = node->next;
            node->next = resortNode;
        }
        head = dummyHead->next;
        delete dummyHead;
        return head;
    ...
    ListNode sortList(ListNode head) {
        ...
        for (int subLength = 1; subLength < length; subLength <<= 1) {
            ...
            while (curr != null) {
                ...
                curr = head2;
                for (int i = 1;
                     i < subLength && curr != null && curr.next != null; i++) {
                    curr = curr.next;
                }
                ListNode next = null;
                if (curr != null) {
